...
          In my company there were 90-100 instances. if i wanted to make any changes in particular instance having some name. Then due to tagging of instances it was easy to search and get that particular instance from list of instances.
...
Ans: Yes, we can create only security group and use them while creating instances. it is not mandatory to create security group while server creation only.
...
  iv) Spot instances are cost-effective choice if you can be flexible about when your application run and if your application can be terminated.
...
           the application servers. i.e. it assumes that all application servers are same with same availability, computing and load handling characteristics.
       ...
       ii) weighted routing : Each target (server, pod, endpoint) gets a numeric weight.

Higher weight = receives more requests.

Example: weight A = 80, weight B = 20 → 80% traffic to A, 20% to B.
...
11. suppose there is one server in one vpc in North Virginia and other server in another VPC in Oregon region. How i can establish the connectivity between this 2 servers ?
...
Add route table entries in both VPCs that point to the peering connection.
...
Ans: OSI model means Open System Interconnection model. In OSI model there are 7 layers that is Application layer, Presentation layer, Session layer, Transport layer, Network layer, Data link layer, Physical layer.
...
 yes. i have created task defination, with all the required specification like number of container i want to run, this is useful for a small scale application.  
...
yes, there are 3 types of provisioner in terraform, local exec remote exec and file provisioner, file provisioner we use when we want to apply the script from our system which is present in same file structure, remote exec we use when we want to apply few commands in the servers which we are going to create, and local exec we use when we want to apply command in our local system after the execution. 
...
19. what is mean by tfstate file ? 
 ...
 Whenever we change the configuration file, it automatically determines which part of your configuration is already created and which needs to be changed with help of state file
...
- suppose we want to deploy some pods on specific node only then we will use node selector.
...
yes. You can use Persistent Volumes (PV) and Persistent Volume Claims (PVC) with a Deployment manifest. 
...
post in jenkins in that post we can mention success of failure as condition and the action that needs to be perforemd after success of failure 
...
31. how many triggers are there in jenkins, to strat theb build process 
1st we can use the cronjob to schedule the trigger 
2nd we can trigger using github webhooks 
3rd we can trigger using upstrim and downstrim of other pipelines. 
...
1. tell me senario where you have used shell scripting 

1st is the take backup of the jenkins home directory and send it to s3 bucket 
#!/bin/bash
...
S3_BUCKET="s3://your-bucket-name/jenkins-backups/"

# step 1: create backup directory if not exists
mkdir -p "$BACKUP_DIR"

# step 2: archive jenkins home
tar -czf "$BACKUP_DIR/$BACKUP_FILE" "$JENKINS_HOME"

# step 3: upload to s3
aws s3 cp "$BACKUP_DIR/$BACKUP_FILE" "$S3_BUCKET"

# step 4: delete local temp backup
rm -f "$BACKUP_DIR/$BACKUP_FILE"


2nd is the senario where we feced multiple ddos attach on our application so we collected the logs from web application firewall and created a shell script to find out unique emailid from logs basicaly i used regular expression to filterit out.

grep -oE "[A-Za-z0-9._%+'\"-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}" <file_name> | sort | uniq | wc -l

3rd is when we had multiple log files from our all the web servers, and our team leads want to find out number of error logs from all the log files. 

 #!/bin/bash 
 for file in /var/log/web/*.log; do 
 ERRORS=$(grep -c "ERROR" "$file") echo "File: $file → Errors Found: $ERRORS" done


2. how can we check if the perticuler file is available in our system or not 
...
3. how do you write a loop that iterates over a list of items 
...
4. how can we rediect the response of any command into a file 

using &> infront of the command like 

ls script.sh &> output.log 

5. how will you schedule a cronjob to run one file on daily night 12 am 
use command 
crontab -e 
then provide cronjob like
0 0 * * * /script.sh 
and save the file 

6. how can we replace the some repeted text from a file. 
use sed command 
